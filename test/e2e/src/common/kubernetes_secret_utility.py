...
def get_kubernetes_secret(api_instance, namespace, secret_name):
    try:
        ...
        sys.exit(f"Error occurred when retrieving secret '{secret_name}': {str(e)}")
...
def watch_kubernetes_secret(api_instance, namespace, secret_name, timeout, callback=None):
    if not callback:
        return
    field_selector = f"metadata.name={secret_name}" if secret_name else ""
    try:
        w = watch.Watch()
        ...
        sys.exit(f"Error occurred when watching kubernetes secret events: {str(e)}")
