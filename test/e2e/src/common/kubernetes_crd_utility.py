...
def get_crd_instance(api_instance, group, version, namespace, plural, crd_name):
    try:
        ...
        pytest.fail(f"Error occurred when retrieving crd information: {str(e)}")
...
def watch_crd_instance(api_instance, group, version, namespace, plural, crd_name, timeout, callback=None):
    ...
    field_selector = f"metadata.name={crd_name}" if crd_name else ""
    try:
        w = watch.Watch()
        ...
        pytest.fail(f"Error occurred when watching crd instance events: {str(e)}")
